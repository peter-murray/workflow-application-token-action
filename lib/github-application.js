...
class GitHubApplication {
  ...
  connect(validSeconds, proxy) {
    ...
    const baseUrl = this.githubApiBaseUrl || process.env.GITHUB_API_URL || 'https://api.github.com';
...
function getProxyAgent(proxy, baseUrl) {
  if (proxy) {
    ...
    if (envProxy && !proxyExcluded(noProxy, baseUrl)) {
...
function proxyExcluded(noProxy, baseUrl) {
  if (noProxy) {
    const noProxyHosts = noProxy.split(',').map(part => {
      try {
        return url.parse(part).host
      } catch (err) {
        return null;
      }
    });
